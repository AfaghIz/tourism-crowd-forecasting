import type { ExplorerPOI, RouteMode } from './types'
...
export function orderByRouteMode(places: ExplorerPOI[], mode: RouteMode): ExplorerPOI[] {
  const copy = [...places]
  switch (mode) {
    case 'fastest':
      return greedyNearest(copy)
    case 'avoid-crowds':
...
function greedyNearest(places: ExplorerPOI[]): ExplorerPOI[] {
  if (places.length <= 1) return places
  const remaining = new Set(places)
  const out: ExplorerPOI[] = []
  let cur = places.find((p) => p.id === 'hagia') ?? places[0]
  remaining.delete(cur)
  out.push(cur)
  while (remaining.size) {
    let best: ExplorerPOI | null = null
    let bestD = Infinity

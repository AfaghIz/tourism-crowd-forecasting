...
function localUxCategory(rawCategory: string, name: string): string {
  const cat = rawCategory.trim()
  const hay = `${cat} ${name}`.toLowerCase()
  if (/museum/.test(hay)) return 'Museum'
  if (/mosque|church|synagogue|cathedral/.test(hay)) return 'Religious'
  if (/palace/.test(hay)) return 'Palace'
  if (/viewpoint|tower|bridge|bosphorus|hill/.test(hay)) return 'Viewpoint'
  if (/market|shopping|street/.test(hay)) return 'Heritage'
  return cat || 'Landmark'
}
...
export function rankLocalPoisNear(origin: LatLng, categoryFilter: string, topK = 10): RankedRecommendation[] {
  ...
  if (categoryFilter) {
    const filtered = list.filter((p) => localUxCategory(p.category, p.name) === categoryFilter)
  ...
  return list
    ...
    .map(({ p, km }) => ({
      ...
      lat: p.lat,
      lng: p.lng,
      category: localUxCategory(p.category, p.name),

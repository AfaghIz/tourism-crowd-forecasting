...
export function recommendationMicroBadges(rec: RankedRecommendation): string[] {
  ...
  const tags = Array.isArray(rec.tags) ? rec.tags.filter((tag): tag is string => typeof tag === 'string' && tag.trim().length > 0) : []
  const dist = typeof rec.distance_km === 'number' ? rec.distance_km : parseFloat(String(rec.distance_km ?? ''))
  const name = String(rec.name ?? '').toLowerCase()
  const out: string[] = []
  if (tags.length > 0) {
    out.push(...tags.slice(0, 3))
  }
  if (cat && out.length < 3 && !out.some((tag) => tag.toLowerCase() === cat.toLowerCase())) out.push(cat)
  if (rawCrowd.includes('low') || rawCrowd === 'l') out.push('Low crowd')
  if (rawCrowd.includes('high') || rawCrowd === 'h') out.push('High crowd')
  if (out.length < 3 && /viewpoint|park|hill|bosphorus|shore/i.test(name)) out.push('Scenic')
  if (out.length < 3 && Number.isFinite(dist) && dist > 1.2 && dist < 4) out.push('Easy detour')
  if (out.length < 3 && cat === 'Religious') out.push('Religious')
  if (out.length < 3 && cat === 'Palace') out.push('Palatial')
  return [...new Set(out)].slice(0, 4)
}

export function recommendationWhyLine(rec: RankedRecommendation): string {
  const crowdReliefRaw =
    typeof rec.crowd_relief === 'number' ? rec.crowd_relief : parseFloat(String(rec.crowd_relief ?? ''))
  const anchorDistRaw =
    typeof rec.distance_to_anchor_km === 'number'
      ? rec.distance_to_anchor_km
      : parseFloat(String(rec.distance_to_anchor_km ?? ''))
  const originDistRaw =
    typeof rec.distance_km === 'number' ? rec.distance_km : parseFloat(String(rec.distance_km ?? ''))

  const bits: string[] = []

  if (Number.isFinite(crowdReliefRaw) && crowdReliefRaw > 0.02) {
    bits.push(`Lower crowd by ${Math.round(crowdReliefRaw * 100)}%`)
  }
  if (Number.isFinite(anchorDistRaw)) {
    bits.push(anchorDistRaw < 1 ? `${Math.round(anchorDistRaw * 1000)} m from your pick` : `${anchorDistRaw.toFixed(1)} km from your pick`)
  } else if (Number.isFinite(originDistRaw)) {
    bits.push(originDistRaw < 1 ? `${Math.round(originDistRaw * 1000)} m away` : `${originDistRaw.toFixed(1)} km away`)
  }

  if (bits.length === 0) return 'Closer match with a calmer outlook'
...
export function poiCardBadges(poi: Poi): string[] {
  const out: string[] = []
  const uxCategory = poi.uxCategory?.trim() || poi.category
  if (/viewpoint|park/i.test(uxCategory)) out.push('Scenic')
  if (/museum|landmark|palace/i.test(uxCategory)) out.push('Don’t rush')
  if (/religious/i.test(uxCategory)) out.push('Cultural icon')
  out.push(uxCategory)
  if (poi.category && poi.category !== uxCategory) out.push(poi.category)
  return [...new Set(out)].slice(0, 3)
}
